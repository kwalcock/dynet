...
CoupledLSTMBuilder::CoupledLSTMBuilder(unsigned layers, unsigned input_dim, unsigned hidden_dim,
...
VanillaLSTMBuilder::VanillaLSTMBuilder() : has_initial_state(false), layers(0),
    input_dim(0), hid(0), dropout_rate_h(0), ln_lstm(false), forget_bias(1.f),
    dropout_masks_valid(false), _cg(nullptr) { }
...
CompactVanillaLSTMBuilder::CompactVanillaLSTMBuilder(unsigned layers, unsigned input_dim,
    ...
    hid(hidden_dim), weightnoise_std(0), dropout_masks_valid(false), _cg(nullptr) {
  unsigned layer_input_dim = input_dim;
